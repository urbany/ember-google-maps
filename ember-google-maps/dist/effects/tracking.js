...
function setupEffect(fn) {
  ...
  // Some map child components are instantiated from async microtasks that may
  // not be followed by another Backburner flush. Prime the effect once so
  // setup runs immediately, then let normal invalidation re-runs flow through
  // the Backburner hook.
  getValue(effect);
  return effect;
}

export { setupEffect, untrack };
