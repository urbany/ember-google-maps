...
function isAdvancedMarkerElement(target) {
  return Boolean(
    ...
    target instanceof google.maps.marker.AdvancedMarkerElement,
  );
}
...
export function addEventListener(
  ...
  if (isDom && target?.nodeName !== 'GMP-INTERNAL-AM') {
    target.addEventListener(listenerEventName, callback, { once: isOnce });

    return {
      name: eventName,
      listener: callback,
      remove: () =>
        target.removeEventListener(listenerEventName, callback, {
          once: isOnce,
        }),
    };
  }

  let listener = google.maps.event[`addListener${isOnce ? 'Once' : ''}`](
    target,
    listenerEventName,
    callback,
  );
